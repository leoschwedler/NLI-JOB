...
import com.usdk.apiservice.aidl.pinpad.UChangePIN;
import com.usdk.apiservice.aidl.pinpad.UPinEntryDelayPinBlock;
...
import com.usdk.apiservice.aidl.tetra.UExternalTetra;
...
public final class DeviceHelper implements ServiceConnection
{
    ...
    private void notifyOnDeviceServiceReady()
    {
        if( serviceListener != null )
        {
            try
            {
                serviceListener.onDeviceServiceReady( deviceService.getVersion() );
    ...
    public UNFCTag getNFCTag() throws IllegalStateException
    {
        IBinder iBinder = new IBinderCreator()
        {
            @Override
            IBinder create() throws RemoteException
            {
                return deviceService.getModule( ModuleName.NFC_TAG_EMULATION, null );
            }

        }.start();

        return UNFCTag.Stub.asInterface( iBinder );
    }
    ...
    public UChangePIN getChangePIN()
    {
        IBinder iBinder = new IBinderCreator()
        {
            @Override
            ...
            {
                Bundle bundle = new Bundle();
                bundle.putString( DeviceServiceData.DEVICE_NAME, "PINPAD_DEVICE_NAME" ); // ??? PINPAD_DEVICE_NAME value is unknown ???

                return deviceService.getModule( ModuleName.CHANGE_PIN, bundle );
            }

        }.start();

        return UChangePIN.Stub.asInterface( iBinder );
    }

    //-----------------------------------------------------------------------------------------------------------------------------------------

    public UExternalTetra getExternalTetra()
    {
        IBinder iBinder = new IBinderCreator()
        {
            @Override
            IBinder create() throws RemoteException
            {
                return deviceService.getModule( ModuleName.EXTERNAL_TETRA, new Bundle() );
            }

        }.start();

        return UExternalTetra.Stub.asInterface( iBinder );
    }

    //-----------------------------------------------------------------------------------------------------------------------------------------

    public UPinEntryDelayPinBlock getPinEntryDelayPinBlock()
    {
        IBinder iBinder = new IBinderCreator()
        {
            @Override
            IBinder create() throws RemoteException
            {
                ...
                return deviceService.getModule( ModuleName.PIN_ENTRY_DELAY_PIN_BLOCK, bundle );
            }

        }.start();

        return UPinEntryDelayPinBlock.Stub.asInterface( iBinder );
    }
    ...
    {
        void onDeviceServiceReady( String version );
    }
    ...
    public String getErrorDetail( int error )
    {
        ...
        switch( error )
        {
            case BaseError.SERVICE_CRASH:
                message = "SERVICE_CRASH";
                break;

            case BaseError.REQUEST_EXCEPTION:
                message = "REQUEST_EXCEPTION";
                break;

            case BaseError.ERROR_CANNOT_EXECUTABLE:
                message = "ERROR_CANNOT_EXECUTABLE";
                break;

            case BaseError.ERROR_INTERRUPTED:
                message = "ERROR_INTERRUPTED";
                break;

            case BaseError.ERROR_HANDLE_INVALID:
                message = "ERROR_HANDLE_INVALID";
                break;
        ...
        return "ERROR \"" + message + "\"" + String.format( " [0x%02X]", error );
    }
